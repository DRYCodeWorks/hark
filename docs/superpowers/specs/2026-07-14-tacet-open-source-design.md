# tacet — packaging `dictate` for public release
...
Ship the working tool to **`github.com/DRYCodeWorks/tacet`** — **private at first**,
...
## Rename: `dictate` → `tacet`

`dictate` is unsearchable and collides broadly. `tacet` is an imperative verb
...
|---|---|
| `src/tacet/` | `src/tacet/` |
| `com.drycodeworks.tacet` | `com.drycodeworks.tacet` |
| `com.drycodeworks.tacet-whisper` | `com.drycodeworks.tacet-whisper` |
| `~/.config/tacet/key` | `~/.config/tacet/key` |
| `~/.hammerspoon/tacet-config.lua` | `~/.hammerspoon/tacet-config.lua` |
| `/tmp/tacet.wav` | `/tmp/tacet.wav` |
| `install-client.sh` | `install-client.sh` |
| (none) | `install-server.sh` |

`X-Tacet-Key` header becomes `X-Tacet-Key`.
...
All personal values move to `~/.config/tacet/config.toml`, read with stdlib
...
from `~/.config/tacet/key` locally, with no SSH involved at all.
...
app and that path is fragile there. `tacet` pastes at the OS cursor, so the pane
...
- **No secret has ever been committed** (verified); the shared key is generated at
  runtime into `~/.config/tacet/key`, mode 600, and is gitignored by living outside
  the repo entirely.

## Migration: this rename breaks the author's working setup, briefly

the author's `tacet` is **currently working and in daily use**. The rename touches every
name it depends on, so the cutover must be deliberate rather than incidental:

- The local checkout moves from `.../DRYCodeWorks/dictate` to `.../DRYCodeWorks/tacet`.
  The **loaded launchd plists point at the old absolute path** and will fail the
  moment it moves.
- The old services (`com.drycodeworks.tacet`, `com.drycodeworks.tacet-whisper`)
  must be `bootout`'d, and the new ones (`com.drycodeworks.tacet`,
  `com.drycodeworks.tacet-whisper`) bootstrapped.
- `~/.config/tacet/key` moves to `~/.config/tacet/key`. **Move it, don't
...
- The client's `~/.hammerspoon/tacet-config.lua` → `tacet-config.lua`, and
  `init.lua`'s symlink is repointed.
- the author's own values (tailnet bind IP, his jargon `prompt`) move out of the code and
  into his `~/.config/tacet/config.toml`. That file is **outside the repo** and is
  never published — which is the entire point.

Implementation must end with **tacet working again on the author's machine**, verified by
...
- ~~Replacing Hammerspoon with a native Swift menubar app. It would be the
  ...
  (issue #2). What changed: installing tacet *claims* `~/.hammerspoon/init.lua`,
  ...
  permission with far less behind it. That argument did not exist when tacet had
