...
/// The tacet service: audio in, transcript out. A stdlib HTTP/1.1 server on the
/// loopback (or a configured private address), replacing FastAPI + uvicorn in
/// `src/tacet/app.py` (issue #4). One user, one request at a time — async buys
...
public enum TacetServerError: Error, CustomStringConvertible {
...
public final class TacetServer {
    /// 0.0.0.0 and :: are every interface; "" is how most socket APIs spell
    /// the same thing. Mirrors WILDCARD_BINDS in src/tacet/plists.py.
    ...
    /// Bind and start serving on a port (default: `config.tacetPort`; pass a
    ...
    public func start(port: UInt16? = nil) throws -> NWListener {
        let portNumber = port ?? UInt16(config.tacetPort)
        guard let p = NWEndpoint.Port(rawValue: portNumber) else {
            throw TacetServerError.bindFailed("invalid port \(portNumber)")
        }

        // REFUSE A WILDCARD BIND. tacet's response is pasted into whatever has
        ...
        guard !Self.wildcardBinds.contains(host) else {
            throw TacetServerError.bindFailed(
                """
                server.bind is "\(config.bindHost)", which listens on every network \
                interface.
                tacet's response is pasted into whatever has focus, so this lets anyone \
                ...
                Set it in ~/.config/tacet/config.toml.
                """)
        }
        ...
        boundHost = host
        logger.info("tacet listening on \(host):\(portNumber)")
        return listener
    }
    ...
    private func handle(_ conn: NWConnection) {
        conn.stateUpdateHandler = { [weak self, weak conn] state in
            ...
            if case .ready = state {
                // Enforce server.bind here rather than on the listener. tacet's
    ...
    private func dictate(_ request: HTTPRequest) -> HTTPResponse {
        ...
        // drive-by CSRF (a CORS-simple request needs no preflight). X-Tacet-Key
    ...
    private func sync<T>(_ body: @escaping () async throws -> T) -> Result<T, Error> {
        ...
        var result: Result<T, Error> = .failure(TacetServerError.bindFailed("unreached"))
        Task {
