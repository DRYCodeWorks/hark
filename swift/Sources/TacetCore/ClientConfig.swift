...
///
/// Distinct from `TacetConfig`, which describes how the SERVER deploys itself.
/// Conflating them is what left the agent hardcoded to
/// `http://127.0.0.1:\(config.tacetPort)/dictate` with the key read from
/// `~/.config/tacet/key` — correct for a single machine, and unusable for the
...
/// too.
///
/// Lives at `~/.config/tacet/client.json`:
///
///     {
...
public struct ClientConfig {
    ...
    /// TACET_CLIENT_CONFIG overrides the path, mirroring the server side's
    /// TACET_CONFIG. Neither FileManager.homeDirectoryForCurrentUser nor
    /// NSHomeDirectory() reliably honours $HOME on macOS — both resolve through
    /// the user database — so `HOME=... tacet agent` silently reads the real
    ...
    public static var path: URL {
        ...
        if let env = ProcessInfo.processInfo.environment["TACET_CLIENT_CONFIG"], !env.isEmpty {
            return URL(fileURLWithPath: env)
        }
        return FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".config/tacet/client.json")
    }
    ...
    /// plus `~/.config/tacet/key` — when the file is absent, so an existing
