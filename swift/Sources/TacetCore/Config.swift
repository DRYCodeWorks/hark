...
/// Server deployment configuration — a port of `src/tacet/config.py`.
///
/// Defaults describe the single-machine setup: bind to loopback, expose
/// nothing. `~/.config/tacet/config.toml` (outside the repo) overrides, exactly
...
public struct TacetConfig {
    public var bindHost: String            // server.bind, default 127.0.0.1
    public var tacetPort: Int               // server.port, default 8911
    ...
    public static var configFile: URL {
        if let env = ProcessInfo.processInfo.environment["TACET_CONFIG"] {
            return URL(fileURLWithPath: env)
        }
        return FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".config/tacet/config.toml")
    }

    public static func load() -> TacetConfig {
        var cfg = TacetConfig()
        ...
        do {
            ...
            if let s = parsed.section("server") {
                ...
                cfg.tacetPort = s.int("port") ?? cfg.tacetPort
            }
            ...
            fatalError("malformed tacet config at \(configFile.path): \(error)")
        }
        return cfg
    }
