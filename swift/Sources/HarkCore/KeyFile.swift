...
/// The shared secret gating POST /dictate. Lives at `~/.config/hark/key`,
...
/// never committed.
public enum KeyFile {
    /// Test-only override so tests don't touch the real `~/.config/hark/key`.
    ...
    public static var path: URL {
        ...
        if let env = ProcessInfo.processInfo.environment["HARK_KEY_FILE"] {
            return URL(fileURLWithPath: env)
        }
        return FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".config/hark/key")
    }

    /// Read and trim the key, or nil if absent/unreadable. A key from the
    /// environment wins over the file, mirroring `config.py`'s `HARK_KEY`.
    public static func load() -> String? {
        if let env = ProcessInfo.processInfo.environment["HARK_KEY"] {
