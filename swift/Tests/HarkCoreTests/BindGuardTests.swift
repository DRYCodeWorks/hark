...
final class BindGuardTests: XCTestCase {
    ...
    func testWildcardBindsAreRefused() throws {
        for host in ["0.0.0.0", "::", "", "  "] {
            let cfg = HarkConfig(bindHost: host, harkPort: 8999)
            let server = HarkServer(config: cfg)
            XCTAssertThrowsError(try server.start(), "bind \(host.debugDescription) must be refused") { error in
                guard case HarkServerError.bindFailed(let message) = error else {
    ...
    func testAPrivateAddressIsAccepted() throws {
        let cfg = HarkConfig(bindHost: "127.0.0.1", harkPort: 8998)
        let listener = try HarkServer(config: cfg).start()
