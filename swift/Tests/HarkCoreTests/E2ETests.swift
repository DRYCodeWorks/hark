...
final class E2ETests: XCTestCase {
    ...
    func testFullDictationRoundTrip() async throws {
        ...
        // 2. Real hark server pointed at the fake whisper.
        let harkPort = TestSupport.freePort()
        let server = HarkServer(
            config: HarkConfig(harkPort: Int(harkPort), whisperPort: Int(whisper.port)),
            key: "e2ekey",
            whisper: WhisperClient(baseURL: "http://127.0.0.1:\(whisper.port)"),
            logger: Logger(label: "test"))
        let listener = try server.start(port: harkPort)
        defer { listener.cancel() }
        XCTAssertTrue(waitForHealth(port: harkPort))

        // 3. Real agent client → server → fake whisper → back.
        let client = DictateClient(
            url: URL(string: "http://127.0.0.1:\(harkPort)/dictate")!,
            key: "e2ekey")
    ...
    func testE2ESilenceReturnsNothing() async throws {
        ...
        // Real hark server pointed at the fake whisper.
        let harkPort = TestSupport.freePort()
        let server = HarkServer(
            config: HarkConfig(harkPort: Int(harkPort), whisperPort: Int(whisper.port)),
            key: "e2ekey",
