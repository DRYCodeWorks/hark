...
/// a mock backend, so no real tacet server is needed.
final class DictateClientTests: XCTestCase {
    ...
    func test401() {
        let outcome = dictate(status: 401, body: #"{"detail":"missing or invalid X-Tacet-Key"}"#)
        XCTAssertEqual(outcome, .failed(.unauthorized("missing or invalid X-Tacet-Key")))
    }
