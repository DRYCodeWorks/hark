...
# laptop, not the transcribing desktop. It builds client/agent/ into
# ~/Applications/hark.app and registers it as a LaunchAgent so it starts at
# login.
#
...
check_signature() {
  ...
  if ! codesign --verify --strict "$APP_DST" 2>/dev/null; then
    doctor_fail "hark.app has a valid signature" \
      "rebuild it: ./client/agent/build-agent.sh && ./install-client.sh"
    return 1
  fi
  local identity
...
check_mic() {
  if [[ ! -f "$MIC_STATUS" ]]; then
    doctor_fail "the agent can reach the microphone" \
      "the agent hasn't probed yet — is it running? (./install-client.sh)"
    return 1
  fi
  local status detail
