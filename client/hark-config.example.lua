-- Template for ~/.hammerspoon/hark-config.lua.
--
-- install-client.sh generates the real file for you (fetches `key` from the
...
-- ~/.hammerspoon/hark-config.lua lives outside this repo entirely, for
-- exactly that reason.

return {
  -- Where hark is listening. Loopback is the single-machine setup, where
  ...
  -- that machine's ~/.config/hark/config.toml.
  server = "http://127.0.0.1:8911/dictate",

  -- The shared secret from the server's ~/.config/hark/key. Sent as the
  -- X-Hark-Key header on every request; a wrong or missing value here is
  ...
  -- Optional. Path to the recorder binary that install-client.sh compiles from
